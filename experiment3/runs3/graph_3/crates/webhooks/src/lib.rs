...
/// (as `\:`, matching `corelib::escape::escape_body`).
...
/// and split on it. Mirrors the escaping done by `escape_body`, where a
/// literal ':' is emitted as `\:` and a literal '\' is emitted as `\\`.
...
mod header_tests {
    ...
    fn does_not_split_on_an_escaped_colon() {
        let pairs = split_headers("to: 10\\:30 report");
        assert_eq!(
            pairs,
            vec![("to".to_string(), "10\\:30 report".to_string())]
        );
    }
}
