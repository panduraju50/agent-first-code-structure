...
/// (i.e. not preceded by an unconsumed backslash).
...
mod header_tests {
    ...
    #[test]
    fn ignores_an_escaped_colon_in_the_value() {
        let pairs = split_headers("to: al\\:ice");
        assert_eq!(pairs, vec![("to".to_string(), "al\\:ice".to_string())]);
    }

    #[test]
    fn skips_a_line_with_no_unescaped_colon() {
        let pairs = split_headers("no colon here\\:");
