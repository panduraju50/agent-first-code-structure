...
/// Find the byte index of the first delimiter ':' in `line`, skipping any
/// ':' that is escaped (preceded by an unescaped '\\'), since an escaped
/// colon is payload content, not a header delimiter.
fn find_delimiter_colon(line: &str) -> Option<usize> {
...
pub fn split_headers(stored_body: &str) -> Vec<(String, String)> {
    stored_body
        .split('\n')
        .filter_map(|line| {
            let idx = find_delimiter_colon(line)?;
...
mod header_tests {
    ...
    fn ignores_an_escaped_colon_in_the_payload() {
        let pairs = split_headers("a\\:b");
        assert_eq!(pairs, Vec::<(String, String)>::new());
    }

    #[test]
    fn splits_on_the_first_unescaped_colon_after_escaped_ones() {
