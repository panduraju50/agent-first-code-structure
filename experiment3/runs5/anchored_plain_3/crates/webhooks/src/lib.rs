...
/// Split a single line on the first ':' that was not escaped (i.e. not
/// preceded by an odd number of backslashes). Mirrors the escaping done by
/// `corelib::escape::escape_body`, where a literal ':' is stored as `\:`.
fn split_first_unescaped_colon(line: &str) -> Option<(&str, &str)> {
...
pub fn split_headers(stored_body: &str) -> Vec<(String, String)> {
    stored_body
        .split('\n')
        .filter_map(split_first_unescaped_colon)
...
mod header_tests {
    ...
    fn ignores_an_escaped_colon_in_the_value() {
        let pairs = split_headers("to: 12\\:30");
        assert_eq!(pairs, vec![("to".to_string(), "12\\:30".to_string())]);
    }

    #[test]
    fn treats_a_colon_after_an_escaped_backslash_as_a_delimiter() {
        let pairs = split_headers("to\\\\: alice");
        assert_eq!(pairs, vec![("to\\\\".to_string(), "alice".to_string())]);
    }
}
