...
/// Like `str::split_once(':')`, but a colon that was escaped (preceded by an
/// odd number of backslashes, per `corelib::escape::escape_body`) is not
...
mod header_tests {
    ...
    fn does_not_split_on_an_escaped_colon() {
        let pairs = split_headers("subject: a\\:b");
        assert_eq!(pairs, vec![("subject".to_string(), "a\\:b".to_string())]);
    }
}
