...
/// (i.e. not preceded by a backslash that itself isn't escaped).
...
mod header_tests {
    ...
    #[test]
    fn ignores_an_escaped_colon_in_the_key() {
        let pairs = split_headers("a\\:b: value");
        assert_eq!(pairs, vec![("a\\:b".to_string(), "value".to_string())]);
    }

    #[test]
    fn does_not_split_further_on_an_escaped_colon_in_the_value() {
        let pairs = split_headers("msg: a\\:b");
        assert_eq!(pairs, vec![("msg".to_string(), "a\\:b".to_string())]);
    }
}
