...
/// Find the byte index of the first ':' in `line` that is not escaped
/// (i.e. not preceded by an odd number of backslashes). `corelib::escape::escape_body`
/// escapes ':' as `\:`, so an escaped colon in a stored value must not be
...
pub fn split_headers(stored_body: &str) -> Vec<(String, String)> {
    stored_body
        .split('\n')
        .filter_map(|line| {
            find_unescaped_colon(line).map(|idx| {
...
mod header_tests {
    ...
    fn ignores_escaped_colons_in_the_value() {
        // corelib::escape::escape_body turns "meet at 10:30" into
        // "meet at 10\:30" — that escaped colon must not be mistaken
        // for the key/value delimiter.
        let escaped = corelib::escape::escape_body("meet at 10:30");
        let line = format!("note:{escaped}");
        let pairs = split_headers(&line);
        assert_eq!(
            pairs,
            vec![("note".to_string(), "meet at 10\\:30".to_string())]
        );
    }
}
