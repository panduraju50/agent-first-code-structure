...
/// Find the byte index of the first ':' in `line` that was not escaped by
/// `corelib::escape::escape_body` (i.e. not preceded by an unescaped '\').
fn find_unescaped_colon(line: &str) -> Option<usize> {
    let mut chars = line.char_indices();
    while let Some((i, c)) = chars.next() {
        if c == '\\' {
            // Skip the escaped character, whatever it is (it cannot itself
            // be an unescaped delimiter).
            chars.next();
            continue;
        }
        if c == ':' {
...
pub fn split_headers(stored_body: &str) -> Vec<(String, String)> {
    stored_body
        .split('\n')
        .filter_map(|line| {
            find_unescaped_colon(line).map(|i| (&line[..i], &line[i + 1..]))
        })
...
mod header_tests {
    ...
    fn does_not_split_on_an_escaped_colon() {
        let pairs = split_headers("note: a\\:b is fine");
        assert_eq!(
            pairs,
            vec![("note".to_string(), "a\\:b is fine".to_string())]
        );
    }

    #[test]
    fn ignores_lines_with_no_unescaped_colon() {
        let pairs = split_headers("just text with escaped\\: only");
