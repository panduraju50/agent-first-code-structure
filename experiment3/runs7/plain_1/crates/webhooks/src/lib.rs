...
/// Find the first ':' in `line` that is not preceded by an unescaped
/// backslash, and split the line there.
...
mod header_tests {
    ...
    fn does_not_split_on_an_escaped_colon() {
        let pairs = split_headers("time\\: 5\\:30: value");
        assert_eq!(
            pairs,
            vec![("time\\: 5\\:30".to_string(), "value".to_string())]
        );
    }
}
