...
/// Find the byte index of the first unescaped ':' in `line`, skipping any
/// ':' (or other character) that is preceded by a backslash escape.
...
mod header_tests {
    ...
    fn does_not_split_on_an_escaped_colon() {
        let pairs = split_headers("to: 12\\:00");
        assert_eq!(pairs, vec![("to".to_string(), "12\\:00".to_string())]);
    }
}
