...
mod header_tests {
    ...
    #[test]
    fn an_escaped_colon_in_the_value_is_not_a_second_delimiter() {
        // "a:b" escapes to "a\:b" (see corelib::escape::escape_body); the
        // stored value still contains a literal ':' character, but it must
        // not be mistaken for the key/value delimiter.
        let pairs = split_headers("to: alice\\:example");
        assert_eq!(
            pairs,
            vec![("to".to_string(), "alice\\:example".to_string())]
        );
    }
}
