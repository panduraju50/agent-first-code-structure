...
/// Find the byte index of the first ':' in `s` that is not escaped with a
/// preceding backslash (i.e. not part of a `\:` sequence).
...
mod header_tests {
    ...
    fn does_not_split_on_an_escaped_colon() {
        let pairs = split_headers("note\\: no colon here");
        assert_eq!(pairs, Vec::<(String, String)>::new());
    }

    #[test]
    fn splits_on_the_first_real_colon_after_an_escaped_one() {
