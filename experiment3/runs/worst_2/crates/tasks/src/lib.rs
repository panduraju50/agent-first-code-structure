...
impl Task {
    /// Render this task's due date in the product's standard timestamp
...
impl TaskStore {
    ...
    pub fn set_due(&mut self, id: &str, due_ts: i64) -> Result<(), String> {
        if due_ts < 0 {
            return Err(format!("due_ts must not be negative: {due_ts}"));
        }
        let task = self
            .tasks
            .iter_mut()
    ...
    pub fn due_within(&self, now: i64, window_secs: i64) -> Vec<&Task> {
        ...
        let mut result: Vec<&Task> = self
            ...
            .iter()
            .filter(|t| !t.done)
            .filter(|t| matches!(t.due, Some(d) if d >= now && d <= end))
            .collect();
