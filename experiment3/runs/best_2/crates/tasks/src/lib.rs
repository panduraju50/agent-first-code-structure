...
use corelib::validate::{validate_timestamp, validate_title};
...
impl Task {
    /// Render this task's due date using the product's standard timestamp
    /// display format, or `None` when there is no due date.
...
impl TaskStore {
    ...
    pub fn due_within(&self, now: i64, window_secs: i64) -> Vec<&Task> {
        ...
        let mut result: Vec<&Task> = self
            ...
            .iter()
            .filter(|t| !t.done)
            .filter(|t| matches!(t.due, Some(due) if due >= now && due <= end))
            .collect();
...
mod tests {
    ...
    fn due_defaults_to_none() {
        let mut s = TaskStore::new();
        let t = s.create("no due date yet").unwrap();
        assert_eq!(t.due, None);
        assert_eq!(t.due_display(), None);
    }
    ...
    fn sets_and_clears_due() {
        let mut s = TaskStore::new();
        let t = s.create("ship it").unwrap();
        s.set_due(&t.id, 1_000).unwrap();
        let got = s.get(&t.id).unwrap();
        assert_eq!(got.due, Some(1_000));
        assert_eq!(got.due_display(), Some(corelib::timefmt::format_ts(1_000)));
    ...
    #[test]
    fn set_due_errors_on_unknown_id() {
        let mut s = TaskStore::new();
        assert!(s.set_due("nope", 100).is_err());
    }

    #[test]
    fn clear_due_errors_on_unknown_id() {
        let mut s = TaskStore::new();
        assert!(s.clear_due("nope").is_err());
    }

    #[test]
    fn due_within_filters_and_sorts() {
        let mut s = TaskStore::new();
        let a = s.create("a").unwrap();
        let b = s.create("b").unwrap();
        let c = s.create("c").unwrap();
        let d = s.create("d, done").unwrap();
        let e = s.create("e, no due").unwrap();

        s.set_due(&a.id, 200).unwrap();
        s.set_due(&b.id, 100).unwrap();
        s.set_due(&c.id, 500).unwrap(); // out of window
        s.set_due(&d.id, 150).unwrap();
        s.complete(&d.id).unwrap();
        let _ = &e;

        let due = s.due_within(100, 100); // window [100, 200]
