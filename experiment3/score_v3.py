...
def sites_updated(root: Path) -> dict:
...
def score(root: Path, hidden: Path) -> dict:
    ...
    r["sites"] = sites_updated(root)
    r["sites_updated"] = sum(1 for v in r["sites"].values() if v)
...
def main() -> int:
    ...
    ap.add_argument("--hidden", default=str(HERE / "hidden" / "hidden_escape.rs"))
    ...
    if a.json:
        ...
        print(f"{res['run']:11} hidden={res['hidden_passed']}/{res['hidden_total']} "
              f"sites={res['sites_updated']}/3 "
              f"[escape={int(s['escape_body'])} inverse={int(s['unescape_body'])} split={int(s['split_headers'])}] "
