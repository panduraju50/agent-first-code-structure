...
pub fn build_digest(tasks: &[Task], seq: u64, opts: &DigestOptions) -> Result<Digest, String> {
    ...
    let end = opts.now + opts.window_days * SECONDS_PER_DAY;
    let mut selected: Vec<&Task> = tasks
        .iter()
        .filter(|t| {
            !t.done
                && t.due
                    .map(|d| d >= opts.now && d <= end)
                    .unwrap_or(false)
        })
        .collect();
    ...
    let lines = page_items
        .iter()
        .map(|t| {
            let title = escape_body(&truncate(&t.title, opts.title_max));
            let due = t.due.map(format_ts).unwrap_or_default();
