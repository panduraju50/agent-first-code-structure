...
impl TaskStore {
    ...
    fn find_mut(&mut self, id: &str) -> Result<&mut Task, String> {
        self.tasks
            .iter_mut()
            .find(|t| t.id == id)
            .ok_or_else(|| format!("no such tasks: {id}"))
    }
}
...
pub fn build_digest(tasks: &[Task], seq: u64, opts: &DigestOptions) -> Result<Digest, String> {
    ...
    let mut selected: Vec<&Task> = tasks
        ...
        .filter(|t| {
            !t.done
                && t.due
                    .is_some_and(|due| due >= opts.now && due <= window_end)
        })
        .collect();
    selected.sort_by_key(|t| t.due);

    let total_pages = page_count(selected.len(), opts.page_size);
    let page_tasks = page_slice(&selected, opts.page, opts.page_size);

    let lines = page_tasks
        .iter()
        .map(|t| {
            let title = truncate(&t.title, opts.title_max);
            let title = escape_body(&title);
            let due = format_ts(t.due.expect("selected tasks have a due date"));
            format!("{} {} | due {}", priority_label(t.priority), title, due)
        })
        .collect();
