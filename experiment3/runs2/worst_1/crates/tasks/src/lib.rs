...
/// Options controlling a task digest.
...
/// A page of upcoming, not-done tasks.
pub struct Digest {
...
/// Build a digest of not-done tasks due within `opts.window_days` days of
/// `opts.now`, inclusive at both ends, earliest due date first.
pub fn build_digest(tasks: &[Task], seq: u64, opts: &DigestOptions) -> Result<Digest, String> {
    ...
    let lines = page_items
        .iter()
        .map(|t| {
            let short_title = truncate(&t.title, opts.title_max);
            let safe_title = escape_body(&short_title);
